...
export function gerarTextosAreas(
  ...
  const ps = (() => {
    ...
    return (
      ...
      `O trabalho agora é de revisão periódica: à medida que o patrimônio cresce e a família evolui, as coberturas precisam acompanhar esse crescimento.`
    );
  })();
  ...
  const fiscal = (() => {
    ...
    if (tipoDeclaracao === 'simplificada') return (
      ...
      `Do ponto de vista tributário, você está pagando o mínimo legal possível para a sua situação atual.`
    );
    ...
    return (
      ...
      `Manter essa eficiência requer atenção anual: renda, aportes e legislação mudam, e a estratégia precisa acompanhar essas mudanças.`
    );
  })();
