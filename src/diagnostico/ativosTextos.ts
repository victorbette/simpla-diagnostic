...
export const ATIVOS_TEXTOS: Partial<Record<string, TextoAtivo>> = {
  ...
  // ── PREVIDÊNCIA ────────────────────────────────────────────────────────────
  previdencia_privada: {
    opiniao: `A previdência privada apresenta vantagens relevantes que merecem ser consideradas em uma estratégia financeira de longo prazo. Do ponto de vista de planejamento sucessório, é um dos poucos instrumentos que permite a transferência de patrimônio sem necessidade de inventário — os recursos são pagos diretamente aos beneficiários, o que reduz custo e tempo em um momento já difícil para a família.\n\nDo ponto de vista fiscal, o PGBL oferece dedução das contribuições na declaração completa do Imposto de Renda, enquanto tanto o PGBL quanto o VGBL permitem o diferimento fiscal — o imposto só incide no momento do resgate, o que no longo prazo pode representar uma vantagem significativa de composição de juros sobre valores que ainda não foram tributados.\n\nA ressalva central, no entanto, está na escolha do fundo onde o dinheiro ficará aplicado. Muitos planos de previdência comercializados por bancos e seguradoras concentram os recursos em fundos com taxas de administração elevadas e performance consistentemente abaixo do CDI — o que pode comprometer boa parte dos benefícios fiscais. A vantagem da previdência só se concretiza quando combinada com um fundo de qualidade, com taxa baixa e gestão eficiente.\n\nEm resumo: o instrumento tem méritos reais — sucessão, diferimento fiscal e disciplina de longo prazo. Mas o resultado final depende diretamente do produto escolhido, e essa escolha exige análise cuidadosa.`,
  },
};
