...
export function SecaoAssetAllocation({
  ...
  function handleCarteiraSave(r: CarteiraResultado) {
    ...
    onResultadoCarteira({
      ...
      planoAcao: r.planoAcao.map((i) => ({
        ...
        portabilidadeDestino: i.portabilidadeDestino,
  ...
  const PERFIS_PILL = [
    ...
    { value: "conservador_moderado", label: "Cons. Moderado" },
  ...
  const perfilCard = (
    <div style={{ background: "white", border: "1px solid #E5E7EB", borderRadius: 12, padding: "12px 16px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "#374151", whiteSpace: "nowrap" as const, flexShrink: 0 }}>
          Perfil do Investidor
        </span>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const, justifyContent: "flex-end" }}>
          {PERFIS_PILL.map((perfil) => {
            const ativo = perfilAtual === perfil.value;
            return (
              <button
                key={perfil.value}
                type="button"
                onClick={() => handlePerfilChange(perfil.value as PerfilRisco)}
                style={{
                  padding: "5px 14px",
                  borderRadius: 99,
                  border: ativo ? "2px solid #2563EB" : "1px solid #E5E7EB",
                  background: ativo ? "#EFF6FF" : "white",
                  color: ativo ? "#2563EB" : "#6B7280",
                  fontSize: 11,
