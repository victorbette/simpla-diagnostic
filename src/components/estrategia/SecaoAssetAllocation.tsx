...
export function SecaoAssetAllocation({
  ...
  const dadosAtual: PizzaSlice[] = CARD_ORDER
    .map((cardId) => ({
      ...
      value: Number(rc.macroAtual[cardId]) || 0,
      cor: CARD_META[cardId].cor,
      brl: ((Number(rc.macroAtual[cardId]) || 0) / 100) * patrimonio,
    }))
    .filter((d) => d.value > 0);
  ...
  const dadosMeta: PizzaSlice[] = CARD_ORDER
    .map((cardId) => ({
      name: CARD_META[cardId].label,
      value: Number(rc.macroMeta[cardId]) || 0,
      cor: CARD_META[cardId].cor,
      brl: ((Number(rc.macroMeta[cardId]) || 0) / 100) * patrimonio,
    }))
    .filter((d) => d.value > 0);
