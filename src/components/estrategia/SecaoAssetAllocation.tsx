    ...
    <>
      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {(() => {
          ...
          if (ativosRec.length === 0) return null;
          return (
            <div style={{ background: "white", border: "0.5px solid #E5E7EB", borderRadius: 12, padding: "20px 24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid #F3F4F6" }}>
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>Como sua carteira deverá ficar</div>
                  <div style={{ fontSize: 12, color: "#6B7280", marginTop: 2 }}>Seleção de ativos recomendados por classe</div>
                </div>
              </div>
              {CARD_ORDER.map((cardId) => {
                const ativos = ativosRec.filter((a) => a.card === cardId);
                if (ativos.length === 0) return null;
                const meta = CARD_META[cardId];
                const pct = Number(rc.macroMeta[cardId]) || 0;
                const brlMeta = (pct / 100) * patrimonioMeta;
                ...
                return (
                  <div key={cardId} style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: cols, padding: "4px 8px", fontSize: 10, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.05em", background: "#F8FAFF", borderRadius: 6, marginBottom: 4 }}>
                      <span>Ativo</span><span>Segmento</span>{isRF && <span>Vencimento</span>}<span style={{ textAlign: "right" }}>R$ Meta</span>
                    </div>
                    {ativos.map((ativo) => (
                      <div key={ativo.id} style={{ display: "grid", gridTemplateColumns: cols, padding: "9px 8px", borderBottom: "0.5px solid #F9FAFB", alignItems: "center" }}>
                        ...
                        <span style={{ fontSize: 10, color: "#374151", background: "#F3F4F6", padding: "2px 8px", borderRadius: 99, display: "inline-block", maxWidth: "fit-content" }}>{ativo.segmento || "—"}</span>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid #E5E7EB", paddingTop: 14, marginTop: 8 }}>
                ...
                <span style={{ fontSize: 16, fontWeight: 700, color: "#111827" }}>{formatBRL(patrimonioMeta)}</span>
              </div>
            </div>
          );
        })()}
