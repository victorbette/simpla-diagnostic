...
export function DocPlanejamentoTributario({ nomeCliente, plan, resultados }: Props) {
  ...
  if (rf) {
    ...
    if (sim) {
      blocos.push({
        chave: "resultado-label",
        grudaNoProximo: true,
        node: <p style={LABEL_SUBSECAO(DOC.ink)}>Resultado</p>,
      });
      blocos.push({
        chave: "resultado-cards",
        node: (
    ...
    // Comparativo de IR — cabeçalho + cards fluem juntos
    blocos.push({
      chave: "comparativo-header",
      grudaNoProximo: true,
      node: (
